...
static void print_help(const char *prog) {
  ...
  printf("Opciones:\n");
  printf("  -p, --palette <name>    Elegir paleta (dark, rose, monokai, nord, "
         "light, blue, ambercat,\n");
  printf(
      "                          dracula, gruvbox, catppuccin, tokyo-night)\n");
  printf("  -f, --font-family <str> Definir tipografía (ej. 'Arial', "
         "'JetBrains Mono')\n");
  printf("  -s, --font-scale <num>  Escalar tamaño de fuentes (ej. 1.2)\n");
  printf("  -e, --export <type>     Exportar slides a 'pdf' o 'png'\n");
  printf("  -er, --export-res <WxH> Resolución de exportación (ej. 1920x1080, "
         "default 1080x1080)\n");
  printf("  -sl, --slide <num>      Seleccionar slide específico para exportar "
         "(0-index)\n");
...
int main(int argc, char *argv[]) {
  ...
  const char *color_bg = NULL, *color_title = NULL, *color_text = NULL,
             *color_accent = NULL;
  int export_png = 0;
  int export_pdf = 0;
  int target_slide = -1;
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--palette") == 0 || strcmp(argv[i], "-p") == 0) {
      if (i + 1 < argc)
        palette_name = argv[++i];
    } else if (strcmp(argv[i], "--font-family") == 0 ||
               strcmp(argv[i], "-f") == 0) {
      if (i + 1 < argc)
        font_family = argv[++i];
    } else if (strcmp(argv[i], "--font-scale") == 0 ||
               strcmp(argv[i], "-s") == 0) {
      if (i + 1 < argc)
      ...
      if (i + 1 < argc) {
        ...
        if (strcmp(type, "pdf") == 0)
          ...
          fprintf(stderr,
                  "Error: Tipo de exportación desconocido '%s'. Use 'pdf' o "
                  "'png'.\n",
                  type);
          return 1;
        }
    ...
    } else if (strcmp(argv[i], "--export-res") == 0 ||
               strcmp(argv[i], "-er") == 0) {
      if (i + 1 < argc) {
  ...
  if (!s)
    return 1;
  if (palette_name)
    slider_set_theme(s, palette_name);
  if (font_family)
    strncpy(s->font_family, font_family, sizeof(s->font_family) - 1);
  if (font_scale > 0.1)
    s->font_scale = font_scale;
  if (color_bg)
    slider_set_color(s, "bg", color_bg);
  if (color_title)
    slider_set_color(s, "title", color_title);
  if (color_text)
    slider_set_color(s, "text", color_text);
  if (color_accent)
    slider_set_color(s, "accent", color_accent);

  int n_slides = slider_get_count(s);
  fprintf(stderr,
          "[slides] %d slide(s) cargados desde %s (tema: %s, font: %s, scale: "
          "%.1f)\n",
